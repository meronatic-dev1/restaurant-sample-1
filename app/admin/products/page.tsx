...
const CATEGORY_COLORS = [
    '#3b82f6', '#60a5fa', '#4ade80', '#fb7185', '#fbbf24',
    '#a78bfa', '#14b8a6', '#f472b6', '#38bdf8', '#fb923c',
];
...
const DIETARY_CFG: Record<string, { color: string; bg: string; border: string }> = {
    ...
    Spicy: { color: '#fb7185', bg: 'rgba(251,113,133,0.08)', border: 'rgba(251,113,133,0.2)' },
};
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', animation: 'fadeUp 0.2s ease both' }}>
            <div style={{ background: '#13131a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 28, width: 480, maxWidth: '95vw', boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10 }}>
                        ...
                        <button onClick={() => setAvailable(a => !a)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, lineHeight: 0 }}>
                            {available
                                ? <ToggleRight size={28} style={{ color: '#3b82f6', filter: 'drop-shadow(0 0 5px rgba(59,130,246,0.4))' }} />
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                    ...
                    <button onClick={save} style={{ flex: 2, padding: '11px 0', background: 'linear-gradient(135deg, #3b82f6, #6366f1)', border: 'none', borderRadius: 10, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 16px rgba(59,130,246,0.35)' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', animation: 'fadeUp 0.2s ease both' }}>
            <div style={{ background: '#13131a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 28, width: 400, maxWidth: '95vw', boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div>
                        ...
                        <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Desserts"
                            style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', padding: '10px 14px', cursor: 'text', borderRadius: 10, borderColor: err ? 'rgba(251,113,133,0.4)' : undefined }}
                            onFocus={e => (e.currentTarget.style.borderColor = err ? 'rgba(251,113,133,0.5)' : 'rgba(59,130,246,0.5)')}
                            onBlur={e => (e.currentTarget.style.borderColor = err ? 'rgba(251,113,133,0.4)' : 'rgba(255,255,255,0.07)')}
                        />
                        {err && <p style={{ margin: '5px 0 0', fontSize: 11, color: '#fb7185' }}>Category already exists</p>}
                    </div>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                    ...
                    <button onClick={save} disabled={!name.trim() || err} style={{ flex: 2, padding: '11px 0', background: !name.trim() || err ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #3b82f6, #6366f1)', border: 'none', borderRadius: 10, color: !name.trim() || err ? 'rgba(255,255,255,0.25)' : '#fff', fontSize: 13, fontWeight: 700, cursor: !name.trim() || err ? 'not-allowed' : 'pointer', fontFamily: 'inherit', boxShadow: !name.trim() || err ? 'none' : '0 4px 16px rgba(59,130,246,0.35)' }}>
...
export default function AdminProductsPage() {
    ...
    const ProductRow = ({ p, i, total }: { p: Product; i: number; total: number }) => (
        ...
        >
            ...
            <td style={{ padding: '12px 16px' }}>
                <span style={{ fontSize: 14, fontWeight: 800, color: '#3b82f6', letterSpacing: '-0.02em' }}>AED {p.price}</span>
            </td>
            <td style={{ padding: '12px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: 36, height: 3, background: 'rgba(255,255,255,0.06)', borderRadius: 99, overflow: 'hidden' }}>
                        <div style={{ height: '100%', borderRadius: 99, background: '#3b82f6', width: `${Math.round((p.orders / maxOrders) * 100)}%`, opacity: 0.8 }} />
                    </div>
            ...
            <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                <button onClick={() => toggle(p.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, lineHeight: 0, transition: 'transform 0.15s' }}
                    ...
                    {p.available
                        ? <ToggleRight size={28} style={{ color: '#3b82f6', filter: 'drop-shadow(0 0 5px rgba(59,130,246,0.5))' }} />
            ...
            <td style={{ padding: '12px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>
                    ...
                    <button onClick={() => deleteProduct(p.id, p.name)} style={{ width: 30, height: 30, borderRadius: 8, background: 'transparent', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.25)', cursor: 'pointer', transition: 'all 0.15s' }}
                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(251,113,133,0.1)'; e.currentTarget.style.color = '#fb7185'; e.currentTarget.style.borderColor = 'rgba(251,113,133,0.2)'; }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'rgba(255,255,255,0.25)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; }}>
                        <Trash2 size={12} />
                    </button>
                </div>
    ...
    return (
        <div style={{ maxWidth: 1200, fontFamily: '"DM Sans", system-ui, sans-serif' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, animation: 'fadeUp 0.4s ease both' }}>
                <div>
                    ...
                    <p style={{ fontSize: 13, margin: 0, color: 'rgba(255,255,255,0.3)' }}>
                        ...
                        <span style={{ color: '#fb7185', fontWeight: 700 }}>{unavCount} unavailable</span>
                        <span style={{ color: 'rgba(255,255,255,0.15)', margin: '0 8px' }}>·</span>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    {/* Add Product */}
                    <button onClick={setShowAddProd.bind(null, true)} style={{ padding: '9px 18px', background: 'linear-gradient(135deg, #3b82f6, #6366f1)', border: 'none', borderRadius: 10, color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, boxShadow: '0 4px 16px rgba(59,130,246,0.35)', transition: 'transform 0.15s, box-shadow 0.15s', fontFamily: 'inherit', letterSpacing: '0.01em' }}
            ...
            {/* ── Bulk action bar ── */}
            {selected.length > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'rgba(59,130,246,0.07)', border: '1px solid rgba(59,130,246,0.22)', borderRadius: 12, padding: '10px 18px', marginBottom: 16, animation: 'fadeUp 0.25s ease both' }}>
                    <span style={{ fontSize: 13, color: '#3b82f6', fontWeight: 700 }}>{selected.length} selected</span>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', borderBottom: isCollapsed ? 'none' : '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginLeft: 10, fontWeight: 600 }}>
                                            ...
                                            {' · '}
                                            <span style={{ color: catAvail > 0 ? '#4ade80' : '#fb7185' }}>{catAvail} active</span>
                                        </span>
                                    </div>
