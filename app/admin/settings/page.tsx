...
export default function AdminSettingsPage() {
    ...
    useEffect(() => {
        // Mock settings data (no backend connected)
        const mockSettings = {
            logoUrl: '',
            bannerUrls: ['', '', ''],
            serviceCharge: 5,
            enableServiceCharge: true,
            enableServiceChargeTakeaway: false,
            deliveryFee: 15,
            taxRate: 5,
        };
        if (mockSettings.logoUrl) setLogoUrl(mockSettings.logoUrl);
        if (mockSettings.bannerUrls) {
            const urls = [...mockSettings.bannerUrls];
            while (urls.length < 3) urls.push('');
            setBannerUrls(urls.slice(0, 3));
        }
        setServiceCharge(mockSettings.serviceCharge);
        setEnableServiceCharge(mockSettings.enableServiceCharge);
        setEnableServiceChargeTakeaway(mockSettings.enableServiceChargeTakeaway);
        setDeliveryFee(mockSettings.deliveryFee);
        setTaxRate(mockSettings.taxRate);
        setLoading(false);
    }, []);
        ...
        <div style={{ maxWidth: 800, fontFamily: '"DM Sans", system-ui, sans-serif' }}>
            ...
            <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: enableServiceCharge ? '#3b82f6' : 'rgba(255,255,255,0.1)',
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <button
                                    ...
                                    style={{
                                        width: 44, height: 24, borderRadius: 12,
                                        background: enableServiceChargeTakeaway ? '#3b82f6' : 'rgba(255,255,255,0.1)',
                                        border: 'none', position: 'relative', cursor: 'pointer',
                                        transition: 'background 0.2s'
                                    }}
                                >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 12, borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 20, marginTop: 10 }}>
                    {message && (
                        <span style={{ fontSize: 13, fontWeight: 600, color: message.type === 'success' ? '#4ade80' : '#fb7185', marginRight: 'auto' }}>
                            {message.text}
                        </span>
                    )}
                    ...
                    <button
                        ...
                        onMouseEnter={e => { if (!saving) { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(59,130,246,0.45)'; } }}
                        onMouseLeave={e => { if (!saving) { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(59,130,246,0.35)'; } }}
                    >
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                    ...
                    <h3 style={{ fontSize: 16, fontWeight: 700, margin: 0, color: '#fb7185' }}>Danger Zone</h3>
                </div>
                ...
                <button
                    ...
                    onMouseEnter={e => { if (!saving) { e.currentTarget.style.background = '#fb7185'; e.currentTarget.style.color = '#fff'; } }}
                    onMouseLeave={e => { if (!saving) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#fb7185'; } }}
                >
