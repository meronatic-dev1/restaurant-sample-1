    ...
    return (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', animation: 'fadeUp 0.2s ease both' }}>
            <div style={{ background: '#13131a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: 28, width: 440, maxWidth: '95vw', boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}>
                ...
                {error && (
                    <div style={{ padding: '10px 14px', background: 'rgba(251,113,133,0.08)', border: '1px solid rgba(251,113,133,0.2)', borderRadius: 10, marginBottom: 16 }}>
                        <span style={{ fontSize: 13, color: '#fb7185', fontWeight: 600 }}>{error}</span>
                    </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div>
                        ...
                        <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. John Doe"
                            style={inputStyle} onFocus={e => (e.currentTarget.style.borderColor = 'rgba(59,130,246,0.5)')} onBlur={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.07)')} />
                    </div>
                    ...
                    <div>
                        ...
                        <input value={email} onChange={e => setEmail(e.target.value)} placeholder="e.g. john@meronatic.com" type="email"
                            style={inputStyle} onFocus={e => (e.currentTarget.style.borderColor = 'rgba(59,130,246,0.5)')} onBlur={e => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.07)')} />
                    </div>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                    ...
                    <button onClick={save} disabled={saving} style={{ flex: 2, padding: '11px 0', background: 'linear-gradient(135deg, #3b82f6, #e55a00)', border: 'none', borderRadius: 10, color: '#fff', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 16px rgba(59,130,246,0.35)', opacity: saving ? 0.7 : 1 }}>
        ...
        >
            <td style={{ padding: '16px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#3b82f6', border: '1px solid rgba(255,255,255,0.08)' }}>
                        <UserIcon size={16} />
                    </div>
                    <div>
            ...
            <td style={{ padding: '16px 20px' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 20, background: u.role === 'ADMIN' ? 'rgba(251,113,133,0.08)' : u.role === 'BRANCH_MANAGER' ? 'rgba(74,222,128,0.08)' : 'rgba(255,255,255,0.05)', border: `1px solid ${u.role === 'ADMIN' ? 'rgba(251,113,133,0.2)' : u.role === 'BRANCH_MANAGER' ? 'rgba(74,222,128,0.2)' : 'rgba(255,255,255,0.08)'}` }}>
                    <Shield size={12} style={{ color: u.role === 'ADMIN' ? '#fb7185' : u.role === 'BRANCH_MANAGER' ? '#4ade80' : 'rgba(255,255,255,0.5)' }} />
                    <span style={{ fontSize: 10, fontWeight: 700, color: u.role === 'ADMIN' ? '#fb7185' : u.role === 'BRANCH_MANAGER' ? '#4ade80' : 'rgba(255,255,255,0.5)', letterSpacing: '0.05em' }}>
                        {u.role.replace('_', ' ')}
                    </span>
                </div>
            </td>
